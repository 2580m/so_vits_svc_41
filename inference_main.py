...
chunks_dict_Path = Path(f'src/plugins/sing/so_vits_svc_41/inference/chunks_temp.json').absolute()
chunks_dict = infer_tool.read_temp(f"{chunks_dict_Path}")
...
def main():
    ...
    for clean_name, tran in zip(clean_names, trans):
        ...
        for spk in spk_list:
            ...
            diffusion, __ = use_diffsuion(shallow_diffusion, only_diffusion, use_spk_mix)
            res_path = f'{svc_Path}/{res}_{key}_{spk}{cluster_name}_{diffusion}_{f0p}.{wav_format}'
            soundfile.write(res_path, audio, svc_model.target_sample, format=wav_format)
            svc_model.clear_empty()
def use_diffsuion(shallow_diffusion, only_diffusion, use_spk_mix):
    """
    根据不同条件配置扩散参数和声音混合选项。
    ...
    # 根据是否使用声音混合，决定spk的值
    # 更新spk配置
    spk = "spk_mix" if use_spk_mix else None
    
    return isdiffusion,spk
def get_res(clean_name: str) -> str:
    pattern = r'(\d+_chunk\d+)\.(wav|mp3)'
